...
def test_validate_server_config_invalid():
    ...
    with pytest.raises(ValueError, match="must have either 'command' for stdio or 'type' and 'url'"):
        validate_server_config("test_server", config)
...
async def test_reload_config_handler():
    ...
    app.state.config_data = {
        "mcpServers": {
            "old_server": {"command": "echo", "args": ["old"]}
        }
    }
    ...
    new_config = {
        "mcpServers": {
            "new_server": {"command": "echo", "args": ["new"]}
        }
    }
